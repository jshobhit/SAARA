...
void fwdBk(){
  if (y < 450 && sm2.step <= 1776 && em1.step <= 6136) {
...
//    }    
  ...
  else if (y < 450 && sm2.step >= 1776 && em1.step <= 6136){
    digitalWrite(sm2.dirpin, HIGH);
    digitalWrite(em1.dirpin, LOW); 
...
void powerOff(){
  ...
  if (em2.step <= 0){
    powerCorrection4 = em2.step - em2.powerHome;
    digitalWrite(em2.dirpin, LOW);      
  }
  else if (em2.step > 0){
  ...
  if (sm2.step <= 0){
    powerCorrection2 = sm2.step - sm2.powerHome;
    digitalWrite(sm2.dirpin, LOW);
  }        
  else if (sm2.step > 0){
  ...
  if (em1.step <= 0){
    powerCorrection3 = em1.step - em1.powerHome;
    digitalWrite(em1.dirpin, HIGH);
  }
  else if (em1.step > 0){
  ...
  while (t < totalPower){
    if (t < powerCorrection) digitalWrite(sm1.pin, HIGH);
    if (t < powerCorrection4) digitalWrite(em2.pin, HIGH);
    if (t < powerCorrection2) digitalWrite(sm2.pin, HIGH);
    if (t < powerCorrection3) digitalWrite(em1.pin, HIGH);
    delayMicroseconds(400); 
    if (t < powerCorrection) digitalWrite(sm1.pin, LOW);
    if (t < powerCorrection4) digitalWrite(em2.pin, LOW); 
    if (t < powerCorrection2) digitalWrite(sm2.pin, LOW); 
    if (t < powerCorrection3) digitalWrite(em1.pin, LOW); 
    delayMicroseconds(400);
    t++; 
  }
...
void testGoToHome (){
  ...
  totalCorrection = max(max(testCorrection, testCorrection2),max(testCorrection3, testCorrection4)); 

  while (w < totalCorrection){
    if (w < testCorrection) digitalWrite(sm1.pin, HIGH);
    // if (w < testCorrection4) digitalWrite(em2.pin, HIGH);
    if (w < testCorrection2) digitalWrite(sm2.pin, HIGH);
    if (w < testCorrection3) digitalWrite(em1.pin, HIGH);
    delayMicroseconds(400); 
    if (w < testCorrection) digitalWrite(sm1.pin, LOW);
    // if (w < testCorrection4) digitalWrite(em2.pin, LOW); 
    if (w < testCorrection2) digitalWrite(sm2.pin, LOW); 
    if (w < testCorrection3) digitalWrite(em1.pin, LOW); 
    delayMicroseconds(400);
    w++; 
  }
  sm1.step = sm1.home; 
...
void testGoToDrink (){
  ...
  drinkTotalCorrection = max(max(drinkCorrection, drinkCorrection2),max(drinkCorrection3, drinkCorrection4)); 

  while (w < drinkTotalCorrection){
    if (w < drinkCorrection) digitalWrite(sm1.pin, HIGH);
    if (w < drinkCorrection4) digitalWrite(em2.pin, HIGH);
    if (w < drinkCorrection2) digitalWrite(sm2.pin, HIGH);
    if (w < drinkCorrection3) digitalWrite(em1.pin, HIGH);
    delayMicroseconds(400); 
    if (w < drinkCorrection) digitalWrite(sm1.pin, LOW);
    if (w < drinkCorrection4) digitalWrite(em2.pin, LOW); 
    if (w < drinkCorrection2) digitalWrite(sm2.pin, LOW); 
    if (w < drinkCorrection3) digitalWrite(em1.pin, LOW); 
    delayMicroseconds(400);
    w++; 
  }
  sm1.step = sm1.home2; 
