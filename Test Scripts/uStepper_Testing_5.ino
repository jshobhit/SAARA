...
void powerOff(){
  ...
  if (em2.step <= 0){
    powerCorrection = em2.step - em2.powerHome;
    while (w < powerCorrection){
      digitalWrite(em2.dirpin, LOW);
      makeStep(em2.pin, 400); 
      w++;
    }        
  }
  else if (em2.step > 0){
    powerCorrection = abs(em2.powerHome - em2.step);
    while (w < powerCorrection){
      digitalWrite(em2.dirpin, HIGH);
      makeStep(em2.pin, 400); 
      w++;
  ...
  if (sm2.step <= 0){
    powerCorrection = sm2.step - sm2.powerHome;
    while (v < powerCorrection){
      digitalWrite(sm2.dirpin, LOW);
      makeStep(sm2.pin, 400); 
      v++;
    }        
  }
  else if (sm2.step > 0){
    powerCorrection = abs(sm2.powerHome - sm2.step);
    while (v < powerCorrection){
      digitalWrite(sm2.dirpin, LOW);
      makeStep(sm2.pin, 400); 
      v++;
  ...
  if (em1.step <= 0){
    powerCorrection = em1.step - em1.powerHome;
    while (r < powerCorrection){
      digitalWrite(em1.dirpin, HIGH);
      makeStep(em1.pin, 400); 
      r++;
    }        
    ...
    powerCorrection = abs(em1.powerHome - em1.step);
    while (r < powerCorrection){
      digitalWrite(em1.dirpin, HIGH);
      makeStep(em1.pin, 400); 
      r++;
